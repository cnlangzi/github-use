#!/usr/bin/env node
/**
 * GitHub Use - GitHub Copilot AI tools for Node.js
 *
 * Unified interface:
 *   import { chat, translate, understandImage, webSearch } from 'github-use/scripts/index.js';
 *
 * CLI usage:
 *   node scripts/index.js chat "hello"
 *   node scripts/index.js image "what is this?" /path/to/image.jpg
 *   node scripts/index.js translate "hello" --to Chinese
 *
 * Authentication:
 *   1. Device Code Flow (default): prints login URL, auto-completes after user auth
 *   2. Direct Copilot Token: set GITHUB_COPILOT_TOKEN env var
 */

import { readFileSync } from 'fs';
import { resolve, extname } from 'path';
import { fileURLToPath } from 'url';

const __dirname = fileURLToPath(new URL('.', import.meta.url));

// Config
const COPILOT_API_HOST = process.env.GITHUB_COPILOT_API_HOST || 'https://api.githubcopilot.com';
const GH_TOKEN = process.env.GH_TOKEN || process.env.GITHUB_TOKEN;
const COPILOT_TOKEN = process.env.GITHUB_COPILOT_TOKEN;

// Fixed headers that VSCode Copilot client sends
const COPILOT_HEADERS = {
  'User-Agent': 'GitHubCopilotChat/0.35.0',
  'Editor-Version': 'vscode/1.107.0',
  'Editor-Plugin-Version': 'copilot-chat/0.35.0',
  'Copilot-Integration-Id': 'vscode-chat',
};

const CLIENT_ID = 'Iv1.b507a08c87ecfe98';

// ─────────────────────────────────────────────
// OAuth Device Code Flow
// ─────────────────────────────────────────────

/**
 * Get Copilot token from GitHub OAuth token
 * Token format: tid=...;exp=...;proxy-ep=proxy.individual.githubcopilot.com;...
 */
function getBaseUrlFromToken(token) {
  const match = token.match(/proxy-ep=([^;]+)/);
  if (!match) return null;
  const proxyHost = match[1];
  return proxyHost.replace(/^proxy\./, 'api.');
}

async function getCopilotToken(githubAccessToken, enterpriseDomain) {
  const domain = enterpriseDomain || 'github.com';
  const copilotTokenUrl = `https://api.${domain}/copilot_internal/v2/token`;

  const resp = await curlRequest(copilotTokenUrl, {
    headers: {
      'Accept': 'application/json',
      'Authorization': `Bearer ${githubAccessToken}`,
    },
  });

  if (!resp.ok) {
    const err = (await resp.json()).error || JSON.stringify(await resp.json());
    throw new Error(`Failed to get Copilot token (${resp.status}): ${err}`);
  }

  return resp.json();
}

/**
 * Make HTTP request via curl (supports HTTP proxy better than Node fetch)
 */
async function curlRequest(url, opts = {}) {
  const { method = 'GET', body, headers = {}, timeout = 30 } = opts;
  const args = ['curl', '-s'];

  if (method !== 'GET') {
    args.push('-X', method);
  }
  args.push('--max-time', String(timeout));

  const httpProxy = process.env.http_proxy || process.env.HTTP_PROXY || null;
  const httpsProxy = process.env.https_proxy || process.env.HTTPS_PROXY || null;
  if (httpsProxy) {
    args.push('-x', httpsProxy);
  } else if (httpProxy) {
    args.push('-x', httpProxy);
  }

  for (const [k, v] of Object.entries(headers)) {
    args.push('-H', `${k}: ${v}`);
  }
  if (body) {
    args.push('-d', body);
  }
  args.push('--', url);

  const { execFileSync } = await import('child_process');
  try {
    const output = execFileSync('curl', args, { encoding: 'utf-8' });
    return { ok: true, status: 200, json: () => JSON.parse(output) };
  } catch (e) {
    const stdout = e.stdout || '';
    const stderr = e.message || '';
    // Try to extract JSON from output
    const match = stdout.match(/\{.*\}/s) || stderr.match(/\{.*\}/s);
    if (match) {
      try {
        return { ok: false, status: e.status || 400, json: () => JSON.parse(match[0]) };
      } catch {}
    }
    return { ok: false, status: e.status || 500, json: () => ({ error: 'request_failed', message: stderr || stdout }) };
  }
}

async function startDeviceFlow(domain) {
  const deviceCodeUrl = `https://${domain}/login/device/code`;
  const resp = await curlRequest(deviceCodeUrl, {
    method: 'POST',
    headers: {
      'Accept': 'application/json',
      'Content-Type': 'application/x-www-form-urlencoded',
    },
    body: new URLSearchParams({
      client_id: CLIENT_ID,
      scope: 'read:user',
    }).toString(),
  });

  if (!resp.ok) {
    throw new Error(`Device code request failed (${resp.status})`);
  }

  return resp.json();
}

async function pollForAccessToken(domain, deviceCode, interval, expiresIn) {
  const accessTokenUrl = `https://${domain}/login/oauth/access_token`;
  const deadline = Date.now() + expiresIn * 1000;

  while (Date.now() < deadline) {
    await new Promise(r => setTimeout(r, interval * 1000));

    const resp = await curlRequest(accessTokenUrl, {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/x-www-form-urlencoded',
      },
      body: new URLSearchParams({
        client_id: CLIENT_ID,
        device_code: deviceCode,
        grant_type: 'urn:ietf:params:oauth:grant-type:device_code',
      }).toString(),
    });

    const data = resp.json();

    if (data.access_token) {
      return data.access_token;
    }

    if (data.error === 'authorization_pending') {
      continue;
    }

    if (data.error === 'slow_down') {
      interval = Math.max(1, interval + 1);
      continue;
    }

    throw new Error(`OAuth error: ${data.error} - ${data.error_description || ''}`);
  }

  throw new Error('Device flow timed out');
}

/**
 * Login to GitHub Copilot via device code flow
 * Returns { access, expires }
 */
async function loginCopilot() {
  const domain = 'github.com';
  const device = await startDeviceFlow(domain);

  console.log('🌐 Open this URL in your browser:', device.verification_uri);
  console.log('🔑 Enter code:', device.user_code);
  console.log('⏳ Waiting for authorization...');

  const githubToken = await pollForAccessToken(
    domain,
    device.device_code,
    device.interval,
    device.expires_in
  );

  const copilotData = await getCopilotToken(githubToken);

  return {
    access: copilotData.token,
    expires: copilotData.expires_at * 1000 - 5 * 60 * 1000, // 5min buffer
    refresh: githubToken,
    baseUrl: COPILOT_API_HOST,
  };
}

// ─────────────────────────────────────────────
// Token Management
// ─────────────────────────────────────────────

// In-memory token cache
let _tokenCache = null;

// Token file for persistence
const TOKEN_FILE = resolve(process.env.HOME || '/tmp', '.config', 'github-copilot', 'token.json');

async function loadTokenFromFile() {
  try {
    const { readFileSync, existsSync } = await import('fs');
    if (existsSync(TOKEN_FILE)) {
      const data = JSON.parse(readFileSync(TOKEN_FILE, 'utf-8'));
      if (data.access && data.expires && Date.now() < data.expires) {
        console.log('📁 Loaded token from', TOKEN_FILE);
        return data;
      }
    }
  } catch {}
  return null;
}

async function saveTokenToFile(tokenData) {
  try {
    const { mkdirSync, writeFileSync, existsSync } = await import('fs');
    const dir = TOKEN_FILE.substring(0, TOKEN_FILE.lastIndexOf('/'));
    if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
    writeFileSync(TOKEN_FILE, JSON.stringify(tokenData, null, 2));
    console.log('💾 Token saved to', TOKEN_FILE);
  } catch (e) {
    console.error('Failed to save token:', e.message);
  }
}

async function getValidToken() {
  if (COPILOT_TOKEN) {
    return COPILOT_TOKEN;
  }

  // Try in-memory cache first
  if (_tokenCache && Date.now() < _tokenCache.expires) {
    return _tokenCache.access;
  }

  // Try file cache
  const fileToken = await loadTokenFromFile();
  if (fileToken) {
    _tokenCache = fileToken;
    return _tokenCache.access;
  }

  // Need to login
  _tokenCache = await loginCopilot();
  await saveTokenToFile(_tokenCache);
  return _tokenCache.access;
}

// ─────────────────────────────────────────────
// API Functions
// ─────────────────────────────────────────────

function inferCopilotInitiator(messages) {
  const last = messages[messages.length - 1];
  return last && last.role !== 'user' ? 'agent' : 'user';
}

function hasCopilotVisionInput(messages) {
  return messages.some((msg) => {
    if (msg.role === 'user' && Array.isArray(msg.content)) {
      return msg.content.some((c) => c.type === 'image');
    }
    if (msg.role === 'toolResult' && Array.isArray(msg.content)) {
      return msg.content.some((c) => c.type === 'image');
    }
    return false;
  });
}

/**
 * Chat with GitHub Copilot LLM
 * @param {string} message - User message
 * @param {Object} opts - Options
 * @param {string} opts.system - System prompt
 * @param {string} opts.model - Model name (default: gpt-4o)
 * @param {number} opts.temperature - Temperature 0-1 (default: 1.0)
 * @param {number} opts.max_tokens - Max tokens (default: 4096)
 * @param {boolean} opts.stream - Enable streaming (default: false)
 * @param {Array} opts.history - Chat history [{role: 'user'|'assistant', content: '...'}]
 * @returns {Promise<{success: boolean, result?: {content: string}, error?: string}>}
 */
async function chat(message, opts = {}) {
  const {
    system = null,
    model = 'gpt-4o',
    temperature = 1.0,
    max_tokens = 4096,
    stream = false,
    history = null
  } = opts;

  const msgs = [];
  if (history) msgs.push(...history);
  msgs.push({ role: 'user', content: message });

  try {
    const token = await getValidToken();

    const body = {
      model,
      messages: msgs,
      max_tokens,
      stream,
    };
    if (system) body.system = system;
    if (temperature !== undefined) body.temperature = temperature;

    const hasImages = hasCopilotVisionInput(msgs);
    const dynamicHeaders = {
      'X-Initiator': inferCopilotInitiator(msgs),
      'Openai-Intent': 'conversation-edits',
    };
    if (hasImages) {
      dynamicHeaders['Copilot-Vision-Request'] = 'true';
    }

    // Use curl for API call (works better with proxy than Node fetch)
    const allHeaders = {
      ...COPILOT_HEADERS,
      ...dynamicHeaders,
      'anthropic-version': '2023-06-01',
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json',
    };

    const resp = await curlRequest(`${COPILOT_API_HOST}/chat/completions`, {
      method: 'POST',
      headers: allHeaders,
      body: JSON.stringify(body),
      timeout: 60,
    });

    if (!resp.ok) {
      const data = resp.json();
      let errorMsg = `HTTP ${resp.status}: ${JSON.stringify(data)}`;
      if (resp.status === 401) errorMsg = `401 Unauthorized: Copilot token expired or invalid. ${errorMsg}`;
      if (resp.status === 403) errorMsg = `403 Forbidden: Check your Copilot subscription. ${errorMsg}`;
      if (resp.status === 404) errorMsg = `404 Not Found: Model '${model}' may not be available. ${errorMsg}`;
      return { success: false, error: errorMsg };
    }

    const data = resp.json();
    // OpenAI format: data.choices[0].message.content
    const content = data.choices?.[0]?.message?.content || '';
    return { success: true, result: { content } };
  } catch (error) {
    return { success: false, error: error.message };
  }
}

/**
 * Understand/analyze image
 * @param {string} prompt - Question about the image
 * @param {string} imagePath - Path to image file or URL
 * @param {Object} opts - Options
 * @param {string} opts.model - Vision model (default: gpt-4o)
 * @param {number} opts.temperature - Temperature 0-1 (default: 0.3)
 * @param {number} opts.max_tokens - Max tokens (default: 300)
 * @returns {Promise<{success: boolean, result?: {content: string}, error?: string}>}
 */
async function understandImage(prompt, imagePath, opts = {}) {
  const {
    model = 'gpt-4o',
    temperature = 0.3,
    max_tokens = 300
  } = opts;

  try {
    const token = await getValidToken();
    let imageData = '';
    let mimeType = 'image/jpeg';

    if (imagePath.startsWith('http://') || imagePath.startsWith('https://')) {
      imageData = imagePath;
    } else {
      const buffer = readFileSync(resolve(imagePath));
      const ext = extname(imagePath).toLowerCase().slice(1);
      const mimeMap = { jpg: 'image/jpeg', jpeg: 'image/jpeg', png: 'image/png', webp: 'image/webp', gif: 'image/gif' };
      mimeType = mimeMap[ext] || 'image/jpeg';
      imageData = buffer.toString('base64');
    }

    // Build content with image
    const content = [];
    if (imageData.startsWith('http')) {
      content.push({ type: 'image_url', image_url: { url: imageData } });
    } else {
      content.push({ type: 'image_url', image_url: { url: `data:${mimeType};base64,${imageData}` } });
    }
    content.push({ type: 'text', text: prompt });

    const msgs = [{ role: 'user', content }];

    const resp = await curlRequest(`${COPILOT_API_HOST}/chat/completions`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
        ...COPILOT_HEADERS,
        'X-Initiator': 'user',
        'Openai-Intent': 'conversation-edits',
        'Copilot-Vision-Request': 'true',
      },
      body: JSON.stringify({
        model,
        messages: msgs,
        temperature,
        max_tokens,
      }),
      timeout: 60,
    });

    if (!resp.ok) {
      const err = resp.json();
      return { success: false, error: `HTTP ${resp.status}: ${JSON.stringify(err)}` };
    }

    const data = resp.json();
    const content_result = data.choices?.[0]?.message?.content || '';
    return { success: true, result: { content: content_result } };
  } catch (error) {
    return { success: false, error: error.message };
  }
}

/**
 * Translate text
 * @param {string} text - Text to translate
 * @param {Object} opts - Options
 * @param {string} opts.to - Target language (default: 'English')
 * @param {string} opts.from - Source language (default: 'auto')
 * @param {string} opts.model - Model name (default: gpt-4o)
 * @returns {Promise<{success: boolean, result?: {content: string}, error?: string}>}
 */
async function translate(text, opts = {}) {
  const { to = 'English', from = 'auto', model = 'gpt-4o' } = opts;
  const systemPrompt = `You are a professional translator. Translate the following text to ${to}${from !== 'auto' ? ` from ${from}` : ''}. Only output the translated text, no explanations.`;
  return await chat(text, { system: systemPrompt, model, temperature: 0.3, max_tokens: 4096 });
}

/**
 * Web search (using model's knowledge base)
 * @param {string} query - Search query
 * @param {Object} opts - Options
 * @param {string} opts.model - Model name (default: gpt-4o)
 * @returns {Promise<{success: boolean, result?: {content: string}, error?: string}>}
 */
async function webSearch(query, opts = {}) {
  const { model = 'gpt-4o' } = opts;
  const systemPrompt = 'You are a helpful assistant with knowledge up to their training date. Answer questions based on your knowledge.';
  return await chat(query, { system: systemPrompt, model, temperature: 0.5, max_tokens: 2048 });
}

// ─────────────────────────────────────────────
// CLI
// ─────────────────────────────────────────────

async function main() {
  const args = process.argv.slice(2);
  if (args.length === 0) {
    console.log('Usage:');
    console.log('  node scripts/index.js chat "message" [--model MODEL] [--stream]');
    console.log('  node scripts/index.js image "prompt" /path/to/image.jpg [--model MODEL]');
    console.log('  node scripts/index.js translate "text" --to Chinese [--from English]');
    console.log('  node scripts/index.js search "query" [--model MODEL]');
    console.log('  node scripts/index.js auth                      # OAuth device code flow');
    console.log('');
    console.log('Environment variables:');
    console.log('  GITHUB_COPILOT_TOKEN - Direct Copilot token (skip OAuth)');
    console.log('  http_proxy / https_proxy - Proxy URL (for auth command)');
    console.log('  GITHUB_COPILOT_API_HOST - API host (default: https://api.githubcopilot.com)');
    console.log('');
    console.log('Models: gpt-4o (default), gpt-4o-mini, claude-sonnet-4.5, etc. (43 models total)');
    process.exit(0);
  }

  const command = args[0];
  const rest = args.slice(1);

  const getOpt = (args) => {
    const result = {};
    const remaining = [];
    const consumed = new Set();
    for (let i = 0; i < args.length; i++) {
      if (consumed.has(i)) continue;
      const opt = args[i];
      if (opt === '--model' && i + 1 < args.length) { result.model = args[i + 1]; consumed.add(i); consumed.add(i + 1); }
      else if (opt === '--stream') { result.stream = true; consumed.add(i); }
      else if (opt === '--to' && i + 1 < args.length) { result.to = args[i + 1]; consumed.add(i); consumed.add(i + 1); }
      else if (opt === '--from' && i + 1 < args.length) { result.from = args[i + 1]; consumed.add(i); consumed.add(i + 1); }
      else if (!opt.startsWith('--')) { remaining.push(opt); consumed.add(i); }
    }
    return { parsed: result, remaining };
  };

  let result;
  switch (command) {
    case 'chat': {
      const msg = rest.find(a => !a.startsWith('--')) || '';
      const { parsed } = getOpt(rest);
      result = await chat(msg, parsed);
      break;
    }
    case 'image': {
      const prompt = rest.filter(a => !a.startsWith('--') && !a.startsWith('/')).join(' ');
      const imagePath = rest.find(a => a.startsWith('/'));
      const { parsed } = getOpt(rest);
      if (!imagePath) { console.error('Error: image path required'); process.exit(1); }
      result = await understandImage(prompt, imagePath, parsed);
      break;
    }
    case 'translate': {
      const text = rest.find(a => !a.startsWith('--')) || '';
      const { parsed } = getOpt(rest);
      result = await translate(text, parsed);
      break;
    }
    case 'search': {
      const query = rest.find(a => !a.startsWith('--')) || '';
      const { parsed } = getOpt(rest);
      result = await webSearch(query, parsed);
      break;
    }
    case 'auth': {
      const tokenData = await loginCopilot();
      console.log('\n✅ Authorization complete!');
      console.log('Token:', tokenData.access.slice(0, 40) + '...');
      console.log('Expires:', new Date(tokenData.expires).toISOString());
      process.exit(0);
    }
    default:
      console.error(`Unknown command: ${command}`); process.exit(1);
  }

  if (result?.success) {
    console.log(result.result.content || '');
  } else {
    console.error('Error:', result?.error || 'Unknown error');
    process.exit(1);
  }
}

export { chat, understandImage, translate, webSearch, loginCopilot, getValidToken };

// Run CLI if executed directly (not when imported as module)
const isMain = process.argv[1] && process.argv[1].includes('index.js');
if (isMain) {
  main().catch(err => { console.error('Fatal error:', err.message); process.exit(1); });
}
